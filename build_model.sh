...
# build_model.sh - Builds the custom bashbot model based on gpt-oss,
# preserving its advanced template for native tool use.

# --- Configuration ---
# Define the base model here for easy changes in the future.
BASE_MODEL="gpt-oss:latest"

# The system prompt is now very simple. It just defines the agent's core
# personality and goal. The Ollama API and the template will handle the tool docs.
SYSTEM_PROMPT="You are Bashbot, a powerful and helpful AI agent. Your primary goal is to assist the user by reasoning about their request and using the tools available to you when necessary to accomplish the task."

# --- Create the Modelfile ---
# We use a Here Document to write the entire Modelfile content.
# The `cat > Modelfile` command will create or overwrite the Modelfile.
# The `<<'EOF'` with single quotes tells Bash to treat the content literally,
# so we don't have to escape all the '$' characters in the template.
cat > Modelfile <<'EOF'
...
# The complex prompt template is copied exactly from the base model.
# This is essential for its tool-using capabilities.
...
# Your custom parameters from the original model
PARAMETER temperature 1
EOF

# Append the SYSTEM prompt using 'tee -a' to avoid escaping issues.
echo "SYSTEM \"\"\"$SYSTEM_PROMPT\"\"\"" | tee -a Modelfile > /dev/null

# --- Build the Model ---
echo "--- Building/updating 'bashbot' model from generated Modelfile... ---"
ollama create bashbot -f ./Modelfile
